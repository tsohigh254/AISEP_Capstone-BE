...
public interface IAiEvaluationService
{
    ...
    Task<ApiResponse<EvaluationStatusResult>> GetEvaluationStatusAsync(int runId);

    /// <summary>Get the full evaluation report (fetch from Python if not cached locally).</summary>
    Task<ApiResponse<EvaluationReportResult>> GetEvaluationReportAsync(int runId);
    ...
    Task<ApiResponse<List<EvaluationStatusResult>>> GetEvaluationHistoryAsync(int startupId);
}
