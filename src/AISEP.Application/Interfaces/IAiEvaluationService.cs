...
public interface IAiEvaluationService
{
    ...
    Task<ApiResponse<EvaluationStatusResult>> GetEvaluationStatusAsync(int runId, int currentUserId = 0, string? currentUserType = null);

    /// <summary>Get the full evaluation report (fetch from Python if not cached locally).</summary>
    Task<ApiResponse<EvaluationReportResult>> GetEvaluationReportAsync(int runId, int currentUserId = 0, string? currentUserType = null);
    ...
    /// </summary>
    Task<ApiResponse<EvaluationReportResult>> GetSourceReportAsync(int runId, string documentType, int currentUserId = 0, string? currentUserType = null);
    ...
    Task<ApiResponse<List<EvaluationStatusResult>>> GetEvaluationHistoryAsync(int startupId, int currentUserId = 0, string? currentUserType = null);
}
