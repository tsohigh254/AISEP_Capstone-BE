...
var app = builder.Build();
    ...
    // [TEMPORARY FIX] Ensure missing mentorship columns and StartupWallet table exist
    using (var scope = app.Services.CreateScope())
    {
        var context = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        try {
            ...
            // Fix StartupWallets: drop old lowercase table and recreate with correct PascalCase
            await context.Database.ExecuteSqlRawAsync(@"
                DROP TABLE IF EXISTS startupwallets CASCADE;
                DROP TABLE IF EXISTS ""StartupWallets"" CASCADE;
                CREATE TABLE ""StartupWallets"" (
                    ""WalletId"" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                    ""StartupId"" integer NOT NULL,
                    ""Balance"" numeric NOT NULL DEFAULT 0,
                    ""TotalRefunded"" numeric NOT NULL DEFAULT 0,
                ...
                CREATE UNIQUE INDEX ""IX_StartupWallets_StartupId"" ON ""StartupWallets"" (""StartupId"");
            ");

            // Fix WalletTransactions: drop old lowercase column, add PascalCase column
            await context.Database.ExecuteSqlRawAsync(@"
                ALTER TABLE ""WalletTransactions"" DROP COLUMN IF EXISTS startupwalletid;
                ALTER TABLE ""WalletTransactions"" ADD COLUMN IF NOT EXISTS ""StartupWalletId"" integer NULL;
                ALTER TABLE ""WalletTransactions"" ALTER COLUMN ""WalletId"" DROP NOT NULL;
            ");

            Log.Information("Applied manual database fixes for mentorship and wallets");
        } catch (Exception ex) {
