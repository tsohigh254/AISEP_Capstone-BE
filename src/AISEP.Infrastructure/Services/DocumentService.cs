...
public class DocumentService : IDocumentService
{
    ...
    private DocumentDto MapToDto(Document d)
    {
        return new DocumentDto
        {
            ...
            // Never expose the raw Cloudinary URL — clients fetch via authenticated proxy.
            FileUrl = string.IsNullOrWhiteSpace(d.FileURL) ? string.Empty : $"/api/documents/{d.DocumentID}/content",
        ...
        int documentId, int userId, CancellationToken ct = default)
    {
        ...
        var history = allVersions.Select(d => new DocumentVersionHistoryDto
        {
            ...
            Version = d.Version,
            Title = d.Title ?? string.Empty,
            FileUrl = string.IsNullOrWhiteSpace(d.FileURL) ? null : $"/api/documents/{d.DocumentID}/content",
            UploadedAt = d.UploadedAt,
        ...
        int documentId, int staffId, DocumentReviewStatus status, string? notes, CancellationToken ct)
    {
        ...
        return ApiResponse<DocumentDto>.SuccessResponse(new DocumentDto
        {
            ...
            Version = doc.Version,
            FileUrl = string.IsNullOrWhiteSpace(doc.FileURL) ? null : $"/api/documents/{doc.DocumentID}/content",
